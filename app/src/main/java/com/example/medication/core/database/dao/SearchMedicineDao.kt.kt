...
interface SearchMedicineDao {
    ...
    @Query("SELECT * FROM search_medicines_cache WHERE name LIKE '%' || :query || '%' ORDER BY name ASC")
    fun searchByName(query: String): Flow<List<SearchMedicineEntity>>

    @Query("SELECT * FROM search_medicines_cache WHERE id = :id LIMIT 1")
    suspend fun getById(id: String): SearchMedicineEntity?
